...
namespace SharpSim.Parser
{
	...
	public class FileParser
	{
		...
		private void ProcessInstructionParts (InstructionBase insn, ArchFileParser.Insn_partContext [] parts)
		{
			foreach (var part in parts) {
				if (part.match_part () != null) {
					...
					if (part.behaviour_part ().type_args () == null) {
						...
						insn.AddPart (new BehaviourPart (
							part.behaviour_part ().BEHAVIOUR ().Symbol.ToASTLocation (filename),
							part.behaviour_part ().name.Text,
							part.behaviour_part ().type_args ().IDENT ().Select (a => a.GetText ()),
							BuildMatchExpression (part.behaviour_part ().match_expr ().match_expr_part ())
						));
					}
				} else {
					throw new NotSupportedException ();
		...
		private Behaviour BuildBehaviour (ArchFileParser.Behaviour_defContext ctx)
		{
			...
			if (ctx.ta != null) {
				foreach (var ta in ctx.ta.IDENT ()) {
					behaviour.AddTypeParameter (ta.GetText ());
				}
			}
		...
		private Helper BuildHelper (ArchFileParser.Helper_defContext ctx)
		{
			...
			if (ctx.prototype ().ta != null) {
				foreach (var ta in ctx.prototype ().ta.IDENT ()) {
					helper.AddTypeParameter (ta.GetText ());
				}
			}
		...
		private Expression BuildExpression (ArchFileParser.Call_expressionContext ctx)
		{
			...
			if (typeargs != null) {
				foreach (var ta in typeargs.IDENT ()) {
					call.AddTypeArgument (ta.Symbol.Text);
				}
			}
