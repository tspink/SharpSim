...
namespace SharpSim.Model.SSA
{
	...
	public class SSAASTVisitor : ASTVisitor
	{
		...
		public SSAASTVisitor (IDiagnostics diagnostics, SSAAction action, InstructionFormat format, RegisterFile file)
		{
			...
			if (action.Prototype.IsGenericPrototype)
				throw new ArgumentException ("Cannot visit an action with a generic prototype");
		...
		public override void VisitHelper (Helper helper)
		{
			...
			foreach (var parameter in helper.Parameters) {
				...
				rootScope.CreateSymbol (parameter.Name, SSAType.FromString (this.action, parameter.Type));
			}
		...
		{
			var prototype = new SSAActionPrototype (SSA.PrimitiveType.Void, call.Name);

			var argumentOperands = new List<TypedSSAOperand> ();
			foreach (var argument in call.Arguments) {
				var operand = ExpressionToOperand (argument);
				argumentOperands.Add (operand);

				prototype.AddParameter (operand.Type);
			}

			foreach (var typeArgument in call.TypeArguments) {
				prototype.AddTypeParameter (SSAType.FromString (currentBlock.Owner, typeArgument));
			}

			SSAAction action;
			if (!CurrentBlock.Owner.Owner.TryGetAction (prototype, out action, false)) {
				if (!CurrentBlock.Owner.Owner.TryGetAction (prototype, out action, true)) {
					this.diagnostics.AddError (call.Location.ToDiagnosticLocation (), $"Action {prototype} does not exist");
					return;
				}
			}
			...
			foreach (var operand in argumentOperands) {
				ssa.AddArgument (operand);
			}
		...
		public override void VisitVariableDeclaration (VariableDeclaration varDecl)
		{
			...
			var symbol = CurrentScope.CreateSymbol (varDecl.Name, SSAType.FromString (this.action, varDecl.Type));
			...
			if (varDecl.Assignment != null) {
				...
				if (ssa.Fixed == SSAStatement.Fixedness.Dynamic)
					symbol.Fixedness = SSAStatement.Fixedness.Dynamic;
		...
		{
			var inputOperand = ExpressionToOperand (castop.Expression);

			var ssa = new CastStatement (inputOperand, SSAType.FromString (this.currentBlock.Owner, castop.Type, false));
			this.currentBlock.AddStatement (ssa);
		}
